...
-   **Simulate Data:** (you can skip this part if you work with an
    ...
    -   the `simdata()` function generates synthetic survival dataset
        with
        -   enrollment time defined by enrollment rate or enrollment
            curve
...
-   **Split Data:**
    -   the `cut_dat()` function cuts the data by a specified time
        point. Only subjects who are enrolled before the specified time
...
-   enrollment curve is defined as 10 pt/month in the first 2 months and
...
We cut the dataset by the time when 80% percent subject are enrolled.
Only subjects who are enrolled before the specified time will be kept in
...
fit_b4_cv <- cv.pwexp.fit(train$followT, train$event, nbreak=4, max_set=100, nsim=nsim)
...
legend('topright',c('0 breakpoints','1 breakpoint','2 breakpoints','3 breakpoints'), lwd=3, col=c('green','blue','red','orange'))
```
<br>
...
```{r}
predicted_boot <- predict(event_model_boot=fit_b2_boot, analysis_time=cut, censor_model_boot=fit_censor_boot, future_enroll=list(enroll_rate=20, total_sample=NROW(dat)-NROW(train)))
```
