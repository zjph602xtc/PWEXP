...
predict.pwexp.fit <- function(event_model, cut_indicator, cut_time, censor_model, n_each=100, future_enroll, seed=1818, plot=F){
  ...
  res <- predict.boot.pwexp.fit(event_model, cut_indicator, cut_time, censor_model, n_each=10, future_enroll, seed=1818, plot=F)
...
predict.boot.pwexp.fit <- function(event_model_boot, cut_indicator=NULL, cut_time, censor_model_boot, n_each=10, future_enroll, seed=1818, plot=F){
  ...
  for (j in 1:n_each){
    for (i in 1:nsim){
      ...
      dat_t$followT_abs <- dat_t$followT_abs+cut_time
      ...
      dat_t_cut$followT_abs <- dat_t_cut$followT_abs-para$time[cut_indicator==1]+cut_time
      dat_pre <- rbind(dat_t, dat_t_cut)

      line_data <- plot_event(time=dat_pre$followT_abs, abs_time = T, add=T, event=dat_pre$event, additional_event=sum(para$event), plot=plot, col='grey')
      line_data_fun <- c(line_data_fun, suppressWarnings(approxfun(line_data$time, line_data$n_event, rule=2)))
