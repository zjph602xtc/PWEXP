...
The ``PwePred`` package provides ``dpwexpm()``, ``ppwexpm()``, ``qpwexpm()``, ``rpwexpm()`` functions to the piecewise exponential distribution:
...
F2 <- ppwexpm(t, rate=c(2, 1, 3), breakpoint=c(0.3, 0.8), lower.tail=T)
...
F1 <- ppwexpm(t, rate=2, lower.tail=T)
lines(t, F1, lwd=2)

# plot quantile functions of both distributions
lines(F1, qpwexpm(F1, rate=2, lower.tail=T), lty=2, lwd=2)
lines(F2, qpwexpm(F2, rate=c(2, 1, 3), breakpoint=c(0.3,0.8), lower.tail=T), col='red', 
      lty=2, lwd=2)
...
exponential distribution $t$ given $t>T$ is \begin{align*}
    ...
    \end{cases}
\end{align*}
The ``PwePred`` package provides ``ppwexpm_conditional()``, ``qpwexpm_conditional()``, ``rpwexpm_conditional()`` functions for conditional piecewise exponential distribution:
```{r fig.show="hold", out.width="50%", out.height="100%", fig.width=6, fig.height=5.8}
# Left Figure ------------------------------------------------------------
...
lines(F2_con, qpwexpm_conditional(F2_con, qT=0.1, rate=c(2, 1, 3), breakpoint=c(0.3,0.8)), 
      lty=2, lwd=2, col='red')
